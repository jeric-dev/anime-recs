...
- A "Data last updated" line under the hero subtitle shows the more recent of the two sync steps below — the Anilist list sync or the MAL stats refresh — so it's always accurate regardless of which one ran last
...
- **Data:** Static `data/anime.json` and `data/tag_descriptions.json`, both generated by `fetch_anime.py`; `data/last_updated.json` (also written by `fetch_anime.py`) and `data/bayesian_constants.json`'s `computedAt` (written by `scrape_mal_stats.js`) back the "Data last updated" indicator
